...
func (a *ScanOwnedOperatorsAction) Run(ctx *maa.Context, arg *maa.CustomActionArg) bool {
	...
	cache, err := readOperatorCache(resolveOperatorCachePathFunc())
	if err != nil {
		log.Error().Err(err).Str("component", scanOwnedOperatorsActionName).Msg("cache read failed")
		return false
	}
	cache = mergeOperatorCache(cache, scanCandidates, owned, time.Now())
	if err := writeOperatorCacheFile(resolveOperatorCachePathFunc(), cache); err != nil {
...
func (a *SelectBestOperatorAction) Run(ctx *maa.Context, arg *maa.CustomActionArg) bool {
	...
	selected, err := selectBestOperator(ctx, candidates, p.ROI, p.MaxSwipes)
	if err != nil {
...
func loadOrRefreshOwnedOperators(
	...
	path := resolveOperatorCachePathFunc()
	...
	needsRefresh := p.Mode == operatorCacheModeRefresh || !operatorCacheCoversCandidates(cache, scanCandidates)
	if !needsRefresh {
...
func selectBestOperator(ctx *maa.Context, candidates []operatorCandidate, roi []int, maxSwipes int) (string, error) {
	ctrl, adaptor, err := operatorController(ctx)
	if err != nil {
		return "", err
	}
